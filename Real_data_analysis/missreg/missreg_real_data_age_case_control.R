...
#cohort.study$strata = as.numeric(factor(cohort.study$instit:cohort.study$stage))
#cohort.study$strata = as.numeric(cohort.study$instit)
cohort.strata.table <- table(cohort.study$rel, cohort.study$instit)

#no.of.strata = length(unique(cohort.study$strata))
...
results.npmle.missreg = matrix(NA, 1000,27)
for(i in 1:1000)
{
  
  # CC = list()
  # for(s in 1:no.of.strata)
  # {
  #   set.seed(i)
  #   R.cases = rbinom(length(which(cohort.study$rel == 1 & cohort.study$strata == s)), 1, sampling.fraction.cases.strata1)
  #   set.seed(i)
  #   R.controls = rbinom(length(which(cohort.study$rel == 0 & cohort.study$strata == s)), 1, sampling.fraction.controls.strata1)
  #   
  #   CC.cases = cbind(cohort.study[which(cohort.study$rel == 1 & cohort.study$strata == s),], R=R.cases) 
  #   CC.controls = cbind(cohort.study[which(cohort.study$rel == 0 & cohort.study$strata == s),], R=R.controls)
  #   CC[[s]] = rbind(CC.cases, CC.controls) 
  # }
  # 
  # CC.study = Reduce(rbind, CC)
  ...
  possible_error = tryCatch({
    fit.npmle <- bin2stg(formula = rel ~ histol*stage + histol*age + stage*age, xstrata=c("instit","stage"), data=data.missreg, xs.includes=TRUE)
  }, error=function(e)e)
  ...
  if(length(possible_error) > 2)
  {
    ...
    if(fit.npmle$error == 0)
    {
      ...
      results.npmle.missreg[i,] = rep(NA, 27) 
    }
  }
...
saveRDS(results.npmle.missreg, "/Users/prosenjitkundu/Desktop/Code_biometrics_github/Real_data_analysis/missreg/missreg_realdata_case_control_w_age_new.rds")
