myoptim <- function(study.list, C, initial_val, threshold, no_of_iter_outer, phase_I_var, phase_II_var, theta_R, correction_factor_est_var, sampling_fraction_est_var)
{
  ...
  correction_factor_var_index = match(correction_factor_est_var, colnames(study.list[[2]]))
  ...
  while(continue)
  {
    ...
    #v_1 = (b1 %*% as.matrix(phase_I) %*% tcrossprod(C_11, as.matrix(phase_I)) %*% diag(l)) + (b2 %*% as.matrix(phase_II) %*% tcrossprod(C_21, as.matrix(phase_I)) %*% diag(l))
    ...
    l[nan_indices] <- 0
    #print(class(b2))
    #v_2 = (b1 %*% as.matrix(phase_I) %*% tcrossprod(C_12, as.matrix(phase_II)) %*% (-diag(l))) + (b2 %*% as.matrix(phase_II) %*% tcrossprod(C_22, as.matrix(phase_II)) %*% (-diag(l)))
    ...
    # Dn = (crossprod(ref_dat, Matrix(diag(w_1), sparse = T)) %*% phase_I %*% C_11 %*% crossprod(phase_I,b1)) +
    #   (crossprod(ref_dat, Matrix(diag(w_1), sparse = T)) %*% phase_I %*% C_12 %*% crossprod(phase_II,b2)) +
    #   (crossprod(ref_dat, Matrix(diag(w_2), sparse = T)) %*% phase_II %*% C_21 %*% crossprod(phase_I,b1)) +
    #   (crossprod(ref_dat, Matrix(diag(w_2), sparse = T)) %*% phase_II %*% C_22 %*% crossprod(phase_II,b2)) 
    # 
    Dn = Dn/(study.list[[4]]^2)
    #sqrt_C = sqrtm(C)
    ...
    # Jn = t(ref_dat) %*% diag(w_1) %*% as.matrix(phase_I) %*% C_11 %*% t(t(ref_dat) %*% diag(w_1) %*% as.matrix(phase_I)) + 
    ...
    Jn = crossprod((ref_dat * w_1), phase_I) %*% tcrossprod(C_11, crossprod((ref_dat*w_1), phase_I)) + 
    ...
    #print(det(J_n))
    if(determinant(Jn)$modulus == Inf | determinant(Jn)$modulus == -Inf)
    { 
    ...
    #print(eigen(Jn)$values)
    beta_new <- beta_old - qr.solve(Jn, Dn, tol = 1e-90)
    #print(D_n_beta_t)
    #print(beta_old)
    #print(beta_new)
    ...
    #print(iter)
    if(eps_inner < threshold_optim || iter > 20)
    {
      continue <- FALSE
      if(iter >= 20 && eps_inner >= threshold_optim)
      {
        status = 0
      }
    }
