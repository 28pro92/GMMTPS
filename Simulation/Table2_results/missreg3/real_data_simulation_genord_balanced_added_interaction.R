...
#glm(rel ~ histol*stage + histol*age + factor(stage==3):age, data = nwtco, family = binomial())
beta_real_data = as.numeric(glm(rel~factor(histol)*factor(stage) + factor(histol)*age + factor(stage)*age , data = nwtco, family = binomial())$coefficients)
...
saveRDS(results.npmle.missreg, file="/Users/prosenjitkundu/Desktop/Code_biometrics_github/Simulation/Table2_results/missreg3/missreg_real_data_simulation_genord_balanced_added_interaction.rds")
