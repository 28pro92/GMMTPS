...
corrcheck(marginal)
library(survival)
nwtco$histol = as.factor(nwtco$histol)
nwtco$instit = as.factor(nwtco$instit)
nwtco$stage = as.factor(nwtco$stage)
nwtco$age = scale(nwtco$age/12)
nwtco.model.matrix = as.data.frame(model.matrix(~ rel + instit * histol * stage * age, data = nwtco))

#reduced_model1 = glm(rel~ histol2 + stage2 + stage3 + stage4 + age + histol2:stage2 + histol2:stage3 + histol2:stage4 + histol2:age + stage3:age , data = nwtco.model.matrix, family = binomial())
reduced_model2 = glm(rel~ histol2 + stage2 + stage3 + stage4 + age + histol2:stage2 + histol2:stage3 + histol2:stage4 + histol2:age + stage2:age + stage3:age + stage4:age , data = nwtco.model.matrix, family = binomial())
beta_real_data = as.numeric(reduced_model2$coefficients)
...
rho_nwtco_matrix = cor(cbind(nwtco$instit, nwtco$histol, nwtco$stage,nwtco$age))
...
saveRDS(sim.results, file="/Users/prosenjitkundu/Desktop/Code_biometrics_github/Simulation/Table2_results/genmeta_realdata_simulation_CC_w_main_effects_phase_I.rds")
CI.L = sim.results[,1:13] - 1.96*sqrt(sim.results[,14:26])
CI.U = sim.results[,1:13] + 1.96*sqrt(sim.results[,14:26])
...
length(which(CI.L[,11] <= beta_real_data[11] & CI.U[,11] >= beta_real_data[11]))
length(which(CI.L[,12] <= beta_real_data[12] & CI.U[,12] >= beta_real_data[12]))
length(which(CI.L[,13] <= beta_real_data[13] & CI.U[,13] >= beta_real_data[13]))
