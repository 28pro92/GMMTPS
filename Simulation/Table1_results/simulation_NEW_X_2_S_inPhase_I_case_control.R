...
Omega = function(beta, study.list, phase_I_var, phase_II_var)
{
  ...
  #fit.R = glm(cohort.study$Y ~ cohort.study$V3 + cohort.study$stage2 + cohort.study$stage3 + cohort.study$stage4 + cohort.study$age, family = binomial())
  fit.R = glm(Y~V3 + V4, data = cohort.study, family = binomial())
  theta.R = as.numeric(fit.R$coefficients)
  #status.converged = fit.R$converged
...
for(i in 1:1000)
{
  ...
  fit.R = glm(Y~ X1.cont + V4, data = cohort.study, family = binomial())
  theta.R = fit.R$coefficients
  
  phase_I_var = c("V2","X1.cont", "V4")
  phase_II_var = c("V2", "X1.cont","V4", "V5")
...
saveRDS(sim.results, file="/Users/prosenjitkundu/Desktop/Code_biometrics_github/Simulation/Table1_results/GENMETA_case_control_table1.rds")

beta.true.matrix = t(matrix(rep(beta.true[-2], 1000), 4, 1000))
apply((sim.results[, 1:4]-beta.true.matrix)^2, 2, mean)
